...
public:
    LidarOdometry(): nh("~") {
        ...
        pub_odom = nh.advertise<nav_msgs::Odometry>("/odom", 100);              //发布关键帧
        pub_each_odom = nh.advertise<nav_msgs::Odometry>("/each_odom", 100);    //发布每一帧的运动增量
    ...
    void run() {
        ...
        buildLocalMap();
        downSampleCloud();
        updateTransformationWithCeres();        // 激光里程求解
        savePoses();                            // 保存位姿
        computeRelative();                      // 计算运动增量：当前帧到前一帧的变换

        // 如果是关键帧
        if(kf) {
            kf_num = pose_cloud_frame->points.size();
            publishOdometry();                  // 发布关键帧odom
            publishCloudLast();                 // 发布去畸变后的边缘点、平面点、全点云
        }
        publishEachOdometry();
        clearCloud();
