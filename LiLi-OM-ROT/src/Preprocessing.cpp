...
private:
    ...
    Eigen::Quaterniond qIMU = Eigen::Quaterniond::Identity();       // 从激光扫描起始 到 激光扫描结束 的姿态
...
public:
    ...
    // 啥时候去畸变已经成了一个函数的能干的事了？
    PointType undistortion(PointType pt, const Eigen::Quaterniond quat) {
        double dt = 0.1;
        int line = int(pt.intensity);       //线束id
        double dt_i = pt.intensity - line;  //相对于第一个点的时间 / 第一个点到最后一个点的整段时间
        ...
        // 按比例插值
        // q_si: 从当前这个点时刻的base_link 到 第1个点的base_link 的姿态变换
    ...
    void processIMU(double t_cur)
    {
        ...
        // 如果队列中还有imu数据时间戳 > t_cur
        // 进行插值处理，得到 t_cur 时刻的姿态
        if(i < imu_buf.size()) {
    ...
    void imuHandler(const sensor_msgs::ImuConstPtr& ImuIn)
    {
        ...
        // solveRotation() 不是已经更新了 gyr_0 了吗
        if (!first_imu)
        {
            first_imu = true;
            double rx = 0, ry = 0, rz = 0;
    ...
    void cloudHandler( const sensor_msgs::PointCloud2ConstPtr &laserCloudMsg)
    {
        ...
        if(idx_imu > 0)
            tmpIdx = idx_imu - 1;
        // imu队列为空、已经处理过的最新一帧imu时间戳 > 激光雷达下一帧的时间戳
        ...
        // 如果imu数据已经初始化完成，则进行姿态解算，解算到下一帧激光扫描的时间戳
        if(first_imu)
        ...
        for (int i = 0; i < cloudSize; i++) {
            ...
            // 插值计算，得到这个点相对于第一个点的时间
            float relTime = (ori - startOri) / (endOri - startOri);
            // point.intensity : 整数部分： 线束id号 | 小数部分：相对于第一个点的时间差
